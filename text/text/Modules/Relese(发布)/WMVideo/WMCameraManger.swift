...
class WMCameraManger: NSObject {
    ...
    func initializeVideoWriter() {
        ...
        guard let writer = try? AVAssetWriter.init(outputURL: URL.init(fileURLWithPath: currentUrl), fileType: .mp4) else {
            error("无法写入视频")
            return
        }
        assetWriter = writer
    ...
    func startRecordingVideo() {
        currentUrl = WMCameraFileTools.wm_createFileUrl("MP4")
